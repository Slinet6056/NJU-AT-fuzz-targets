...
if [ -d "$AFLPP_DIR/testcases/images/jpeg" ]; then
  cp "$AFLPP_DIR/testcases/images/jpeg"/* "$RAW_SEEDS_DIR/"
else
  echo "Error: AFL++ testcases directory not found at $AFLPP_DIR/testcases/images/jpeg"
  exit 1
fi

# Copy project's own test images
if [ -d "$SUBJECT_DIR/testimages" ]; then
  cp "$SUBJECT_DIR/testimages"/*.jpg "$RAW_SEEDS_DIR/" 2>/dev/null
  cp "$SUBJECT_DIR/testimages"/*.jpeg "$RAW_SEEDS_DIR/" 2>/dev/null
else
  echo "Warning: Project test images not found at $SUBJECT_DIR/testimages"
  exit 1
fi

# Check if we have any seeds
if [ ! "$(ls -A "$RAW_SEEDS_DIR")" ]; then
  echo "Error: No seed files found. Please provide at least one JPEG file as seed."
  exit 1
fi
...
"$AFLPP/afl-cmin" -i "$RAW_SEEDS_DIR" -o "$CMIN_SEEDS_DIR" -- "$TARGET_DIR/djpeg" @@
...
if [ "$ENABLE_AFL_TMIN" = "1" ]; then
  ...
  for seed in "$CMIN_SEEDS_DIR"/*; do
    if [ -f "$seed" ]; then
      ...
      "$AFLPP/afl-tmin" -i "$seed" -o "$TMIN_SEEDS_DIR/$seed_name" -- "$TARGET_DIR/djpeg" @@
    fi
  done
else
