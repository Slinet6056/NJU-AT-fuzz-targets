...
if [ -d "$AFLPP_DIR/testcases/others/pcap" ]; then
  cp "$AFLPP_DIR/testcases/others/pcap"/* "$RAW_SEEDS_DIR/"
else
  echo "Error: AFL++ testcases directory not found at $AFLPP_DIR/testcases/others/pcap"
  exit 1
fi

# Copy project's own test files
if [ -d "$SUBJECT_DIR/tests" ]; then
  find "$SUBJECT_DIR/tests" -type f -name "*.pcap" -exec cp {} "$RAW_SEEDS_DIR/" \;
  find "$SUBJECT_DIR/tests" -type f -name "*.cap" -exec cp {} "$RAW_SEEDS_DIR/" \;
else
...
if [ ! "$(ls -A "$RAW_SEEDS_DIR")" ]; then
  echo "Error: No seed files found. Please provide at least one PCAP file as seed."
  exit 1
fi
...
if [ "$AFL_USE_TMIN" = "1" ]; then
  ...
  for seed in "$CMIN_SEEDS_DIR"/*; do
    if [ -f "$seed" ]; then
      ...
      "$AFLPP/afl-tmin" -i "$seed" -o "$TMIN_SEEDS_DIR/$seed_name" -- "$TARGET_DIR/tcpdump" -nr @@
    fi
  done
else
